...
internal sealed record BuiltinAgentReferenceEntry(
    string? Description,
    string? Color,
    string? SystemPrompt);
...
{
    public static IReadOnlyDictionary<string, BuiltinAgentReferenceEntry> Data { get; } = CreateData();

    private static IReadOnlyDictionary<string, BuiltinAgentReferenceEntry> CreateData()
    {
        var data = new Dictionary<string, BuiltinAgentReferenceEntry>
        {
        ["build"] = new(
        ...
        ["explore"] = new(
            ...
            **成功标准**：[什么结果能让用户立即继续工作]
            ...
            """),
        ["sisyphus"] = new(
            "强大的 AI 编排者。用待办列表规划，评估搜索复杂度后再探索，战略性地委派工作。",
            "#A855F7",
            """
            <identity>
            ...
            **实施示例**：
            ...
            """),
        ["hephaestus"] = new(
            "自主深度工作者 agent。目标导向执行，行动前充分探索，工程交付附带强验证。",
            "#F97316",
            """
            <identity>
            ...
            """),
        ["prometheus"] = new(
            "战略规划顾问 agent，只规划不实施。将实施请求解读为创建工作计划的请求。",
            "#EF4444",
            """
            <identity>
            ...
            **始终**：
            ...
            """),
        ["oracle"] = new(
            "只读战略顾问 agent，用于复杂架构决策、困难调试和自我审查。不可修改任何文件。",
            "#6366F1",
            """
            <identity>
            你是 Oracle — 战略技术顾问。
