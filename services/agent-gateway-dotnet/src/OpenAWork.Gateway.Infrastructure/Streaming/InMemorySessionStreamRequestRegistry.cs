...
public sealed class InMemorySessionStreamRequestRegistry : ISessionStreamRequestRegistry
{
    ...
    private readonly ConcurrentDictionary<string, object> _sessionLocks = new(StringComparer.Ordinal);

    public SessionStreamInFlightRequest? Get(string sessionId, string clientRequestId)
    {
        lock (GetSessionLock(sessionId))
        {
            return GetNoLock(sessionId, clientRequestId);
        }
    }
    ...
    public SessionStreamInFlightRequest? GetAnyForSession(string sessionId, string userId, string? excludeClientRequestId = null)
    {
        lock (GetSessionLock(sessionId))
        {
            return GetAnyForSessionNoLock(sessionId, userId, excludeClientRequestId);
        }
    }
    ...
    public SessionStreamRegistrationResult RegisterOrGetConflict(string sessionId, string userId, string clientRequestId, CancellationTokenSource cancellation)
    {
        lock (GetSessionLock(sessionId))
        {
            ...
            var anySessionConflict = GetAnyForSessionNoLock(sessionId, userId, clientRequestId);
    ...
    {
        SessionStreamInFlightRequest? request;
        lock (GetSessionLock(sessionId))
        {
            request = GetNoLock(sessionId, clientRequestId);
    ...
    public async Task<bool> StopAnyAsync(string sessionId, string userId, CancellationToken cancellationToken)
    {
        ...
        lock (GetSessionLock(sessionId))
        {
            request = GetAnyForSessionNoLock(sessionId, userId);
            if (request is null)
            {
                return false;
            }
    ...
    private SessionStreamInFlightRequest? GetNoLock(string sessionId, string clientRequestId)
        => _requests.GetValueOrDefault(BuildKey(sessionId, clientRequestId));

    private SessionStreamInFlightRequest? GetAnyForSessionNoLock(string sessionId, string userId, string? excludeClientRequestId = null)
        => _requests.Values.FirstOrDefault((request) =>
