...
- **单条 bash 的实时输出（tool_progress 单元素通道）**：可流式工具（bash）的滚动 stdout 通过 `routes/single-tool-live-output.ts` 的 `buildSingleToolPartialOutputWriter` 写成 `tool_progress` 事件的**单元素 `subTools`**（`index:0` / `status:'running'` / `partialOutput` / `totalCount:1`），前端 `applyStreamToolProgress` → `_batchProgress` → 终端卡实时渲染（与 batch 子行同源、同一份数据契约，**不新增协议事件**）。接入点两个：`routes/stream.ts` 的 `executeToolCalls` 单条工具路径、`routes/stream-runtime.ts` 的 `resumeApprovedPermissionRequest`（批准恢复无 WS/SSE，走 `publishSessionRunEvent`）。节流在 `tools/bash-tools.ts`（80ms 窗口 + 尾包 + `MAX_OUTPUT_BYTES` 上限），网关侧不做二次缓冲。单测：`__tests__/routes/single-tool-live-output.test.ts`。
...
- **权限批准的续跑必须先注册运行线程**（`resumeApprovedPermissionRequest`，`src/routes/stream-runtime.ts`）：被批准的工具执行期间（可能持续数秒）续跑已经在发布 `terminal_*` / `tool_result` 事件，而 `/stream/active`、`/stream/attach`、`/status` 的 activeStream 投影都依赖 `session_runtime_threads` 才报告活跃流。运行线程必须在**执行被批准的工具之前**注册（5s 心跳 + `finally` 清理）；`continueFromApprovedToolResult` 内部仍会 upsert/清理同一线程（语义一致），外层心跳覆盖「工具执行完成 → 模型轮注册线程」之间的空档。若晚注册，客户端在批准后约 100ms 的 attach 会拿到「无活跃流」并放弃重试，续跑输出在界面上永远接不上（表现为「批准后卡住、要整页刷新」）。验收：`verify-permission-resume-runtime-thread.ts`（已接入 `test:batch-permission`）。
- **停止 = 整段断掉（连带作废待审批/待回答）**：`/sessions/:id/stream/stop`、`/stream/stop-active`、`/sessions/:sessionId/tasks/:taskId/cancel`（子任务取消）在 abort 在途流（可能为空）之后，必须分别用**回合级**（`cancelPendingPermissionRequestsByClientRequest` / `cancelPendingQuestionRequestsByClientRequest`）、**会话级**（`...ForSession`，含 `deciding`）原语作废 pending 交互、把对应 `permission_asked` 未读通知标为已读（`markPermissionNotificationsReadForSession`），并 `reconcileSessionRuntime` 收敛状态。只 abort 不清理 pending 时，`reconcileSessionStateStatus` 会因为 pending 交互把会话永久留在 `paused`——现象是「停止后审批卡仍挂着，停止与审批互相卡住」。父级 abort 的子树级联（`stream.ts` 的 `cancelDescendantSessionStreams` 之后）同样要清理各子会话的 pending 交互，否则父会话已停止、暂停中的子会话永远停在 `paused`（动态 import 规避 `stream.ts ↔ permissions/questions` 静态循环）。验收：`verify-stop-cancels-pending-interactions.ts`（已接入 `test:batch-permission`）与 `src/__tests__/routes/task-cancel-pending-cleanup.test.ts`。
- **bash 命令的工作区快照/diff 不得逐文件 spawn 进程**：`captureWorkspaceReconcileSnapshot`（`src/workspace/workspace-reconcile.ts`）在每条 bash 命令前后各跑一次，旧实现会对**每个变更文件**各起一个 `git show HEAD:<path>`（POSIX 上 `uv_spawn` 在主线程同步执行），脏工作区（几百～几千个变更文件）时一次命令就能让网关事件循环硬阻塞 1 秒以上（实测 2000 个变更文件 = 1098ms）——现象是「执行 bash 时桌面端整页卡死 / 网页端接口全部阻塞」。现在快照阶段**只读工作区内容、不读 HEAD**（`canReadHead` 仅记录语义），HEAD 基线由 `collectWorkspaceReconcileDiffs` 仅在「一侧缺失」（命令新增/删除文件）时按需批量读取，并发上限 8（`HEAD_CONTENT_READ_CONCURRENCY`）。不许把 HEAD 读取挪回快照阶段的 `Promise.all`（并发 spawn 风暴正是阻塞源）。验收：`src/__tests__/workspace/workspace-reconcile.test.ts`（用 PATH 上的 git 包装脚本断言快照阶段零 `show`、已脏文件零重复读取）。
- **终端输出进度必须节流落库**（`src/session/session-terminal-registry.ts`）：PTY 高输出（`yes` / `cat` 大文件 / 构建日志）会以每秒数千块到达，`appendDeltaInternal` **不得逐块同步写 `session_terminals`**（单次 UPDATE ≈0.05ms，50k 块累计 ≈2.4s 事件循环占用 → 整机卡死）。写法：逐块只更新内存（ring/tail/totalBytes）并标记 `pendingPersist`，由节流窗口 `flushEmit`（≤`OUTPUT_EMIT_THROTTLE_MS`）统一落库；`markTerminalExited` 写最终值；`getTerminal` 用内存态覆盖 `outputTail`/`outputBytesTotal` 保证读新鲜。低延迟的 `notifyImmediateOutput`（WS 实时流）保持逐块投递，不受节流影响。
- **「保留最近 N 行」的清理必须走主键边界删除**（`src/infra/sqlite-retention.ts` 的 `deleteRowsBeyondMostRecent`）：禁止 `DELETE ... WHERE id NOT IN (SELECT id ... ORDER BY id DESC LIMIT N)`——`NOT IN` 子句会把整表物化进临时 B-tree，在数百 MB 的表上每次 prune（写路径上每 N 次写入触发）都同步阻塞事件循环。全局清理点已统一：`audit_logs` / `event_log` / `permission_decision_logs` / `request_workflow_logs`；`session_run_events` / `session_entry` 的 per-session 清理本身走既有复合索引，保持现状。新表沿用该 helper（`INTEGER PRIMARY KEY` 用 `id`，UUID 主键用 `rowid`）。
- **`audit_logs` 必须有索引**：`idx_audit_logs_session_created(session_id, created_at DESC)` + `idx_audit_logs_created_at(created_at DESC)`（`db.ts` 迁移）。`/settings/dev-logs` 会 `JOIN sessions ... ORDER BY created_at DESC`，缺索引时在 100MB 级表上全表扫描 + 临时排序（同步阻塞）。
- **`sessions(user_id, updated_at DESC)` 必须有索引**（`idx_sessions_user_updated`，`db.ts` 迁移）：会话列表（侧边栏、`/sessions` 路由）是 `WHERE user_id = ? ORDER BY updated_at DESC LIMIT/OFFSET`，此前无可用索引 → 全表扫描 + 临时排序（会话多时同步阻塞）。`EXPLAIN QUERY PLAN` 应显示 `SEARCH sessions USING INDEX idx_sessions_user_updated`。
- **`/sessions/:id/recovery` 的子会话消息必须跟随父请求 turnLimit**（`sessions.ts` 的 `buildSessionRecoveryReadModel`）：未加限时每个子会话全量读消息（N 子会话 × 全历史），大任务树上是同步阻塞。`/recovery` 只把子会话当预览，需要完整子会话转录的入口（子会话详情面板 / 导出）走 `GET /sessions/:id`，不要给 recovery 的子会话开全量口子。
...
- **子代理结果的提取口径（对齐参考库 `SubagentCompletion.text`）**：同步 task 路径只回传子代理**最后一条有文本的 assistant 消息**（`extractLatestChildSessionSummary`，经 `getChildSessionSummary`），扫描窗口为**最近 20 条**消息且只读 `statuses: ['final']`（跳过 error 消息，对齐参考库的 `limit:20` + `message.error === undefined` 判定）；**禁止**再把整个子会话文本 + 工具输出全量拼进父会话工具结果（那是 200k 级上下文膨胀源）。`background_output` **默认也只回摘要**（任务状态 + `getChildSessionSummary`），逐条消息必须显式传 `full_session: true`；full_session 有 **50k 字符总预算**（超出从最早处淘汰并回报 `omittedMessages`）且单条消息正文另有 **20k 字符硬上限**。`buildTaskToolTerminalMessage`（`task/delegated-task-display.ts`）与 `buildCallOmoAgentSyncOutput`（`tools/call-omo-agent-output.ts`）的同步结果同样以 `<subagent sessionID state>` 包裹（对齐参考库前台模型可见内容，state 用上游 wire 词表；保留 `task_id` resume 提示行），正文另有 **20k 字符上限**。
- **每轮固定开销收敛（2026-09-24，对齐 opencode v2.0.15）**：① 能力目录 `buildCapabilityContext` 的「聊天可调用工具」区**只列名称**、「系统 Skills」在 `Skill` 工具可见时只列名称并指向工具描述（目录从 ~53k 字符降到 ~9k），并且**改为 instructions 式 stable system 槽位**（`capabilityCatalogPrompt`，不再挂在每条用户消息的 synthetic part 上——字节在会话内稳定、能力变化时下一轮整段刷新，对齐参考库 instructions 的更新形状）；② 工具输出的**模型视图**上限改为参考库语义 **2000 行 / 50 KiB 字节**（`tools/tool-output-truncator.ts`；按字节而非字符记账，中文场景与参考库一致），超限时**落盘保留更长结果**（存储上限 200k **字符**，`STORAGE_DEFAULT_MAX_CHARS`）并在截断提示里引导模型用 `read_tool_output` + toolCallId 取回——`truncateToolOutput` 是模型视图、`truncateToolOutputUniversal` 是落盘视图，二者不可混用；③ `session_read` 默认只回最近 20 条并折叠长文本（`full: true` 才返回完整文本，`limit` 上限 500，transcript 审计行上限 200）；④ session memory 提取**默认关闭**（参考库无此机制；需要时 `OPENAWORK_ENABLE_SESSION_MEMORY_EXTRACTION=1`），开启后只发送 `lastSessionMemoryMessageId` 之后的**增量**（预算 20k tokens；水位只推进到实际发送的最后一条）；⑤ 请求级**内联图片预算**（`message/image-request-budget.ts`，对齐参考库 `boundImages`）：单次请求内联图片总字节 > 25 MiB 时从最早开始淘汰到 15 MiB（`OPENAWORK_IMAGE_BUDGET_TRIGGER_BYTES` / `..._TARGET_BYTES` 可覆盖，0 关闭），远程 URL / 仅 artifactId 不计入。
- **工具折叠（模型可见工具面预算，对齐参考库 Code Mode 目录的形状）**：普通 chat 会话（非 team / 非渠道）默认启用（`OPENAWORK_DISABLE_TOOL_FOLDING=1` 回退完整面）：`tools/tool-folding.ts` 把工具面拆成**直连工具**（`DIRECT_TOOL_NAMES`，原生 JSON Schema）与**折叠工具**（显式 `DEFERRED_TOOL_NAMES` + `mcp__*`；未知/新增工具默认直连），折叠面只在 system 提示词里渲染**紧凑签名 + 120 字符描述**的目录（8k 字符 ≈ 2000 tokens 预算，注入 `buildTwoPartSystemPrompts` 的 stable 段 `toolCatalogPrompt` 槽位）。模型用 `tool_search({query})` 查完整 schema、`tool_invoke({tool,arguments})` 调用。**安全不变量**：① 网关每轮把过滤后的可见工具名写入 `sessions.metadata_json.toolInvokeAllowlist`（`session/tool-invoke-allowlist.ts`），沙箱在**解包前**校验内层工具必须在名单内且 `isGatewayToolEnabledForSessionMetadata` 为真；② 解包发生在 `executeToolCall` 最前端（legacy rewrite 之后、Claude 名称归一之前），因此**白名单 / 会话启用 / 插件 hook / 权限阶梯 / 审批 / 执行全部按内层工具名走**，deny/ask/team/channel/clarify 语义与直接调用完全一致（`tool_invoke`/`tool_search` 只做「模型可见名必须有运行期路径」的 parity 登记，真实权限判定仍在解包后的内层工具上；`tool_search` 为只读形态）；③ 禁止嵌套 `tool_invoke`/`tool_search`；④ `execute()` 的索引失效按内层工具名执行（`resolveInvokeEffectiveToolName`），否则折叠后的写操作对 `@` 索引不可见。验收：`__tests__/tools/tool-folding.test.ts`、`__tests__/tools/tool-sandbox-tool-invoke.test.ts`。
- **工具输出全文落盘（spill，2026-09-24）**：超限工具结果的**全文**写入 `<dataDir>/tool-outputs/<sessionId>/<toolCallId>.txt`（`tools/tool-output-spill.ts`，对齐参考库 `tool-output.ts` 的 full-content-to-file 语义），数据库仍存截断版（200k 字符）；`read_tool_output` 优先读 spill 文件（读不到回退数据库输出，并在 `note` 标注「全文来自落盘文件」），因此模型侧取回不再受持久化上限约束。落盘触发点在 `buildToolResultContent`（需要 `sessionId`，4 个持久化调用点均已传）；路径只接受 `[A-Za-z0-9_-]` 且必须落在 spill 根目录之下（防目录穿越）；写失败只告警、不影响工具执行；会话删除时清理该会话目录，网关启动时兜底清理 7 天前的目录。验收：`__tests__/tools/tool-output-spill.test.ts`。
- **工作区上下文会话级冻结（2026-09-24）**：`buildWorkspaceContext` 含文件树 + AGENTS.md / README / 规则文件，此前**每请求重读**——根级文件增删、或 AGENTS.md 被编辑，都会改变 stable system 前缀字节，导致「工具定义 + 全部 system 段」的 prompt-cache 前缀**整段失效**（现象：某一轮 fresh input 突然暴涨）。现在经 `session/workspace-context-snapshot.ts` 的 `resolveFrozenWorkspaceContext` 冻结为**会话级快照**（存 `sessions.metadata_json.workspaceCtxSnapshot`，含 `workspacePath` 用于会话迁移时失效），对齐参考库 instructions 的「会话基线」语义；工作区变化从**下一个会话**生效，模型仍可用 `list` / `glob` 取最新目录。开关：`OPENAWORK_DISABLE_WORKSPACE_CTX_FREEZE=1` 回退为每请求重建。验收：`__tests__/session/workspace-context-snapshot.test.ts`。
- **prompt-cache 稳定性不变量（2026-09-24，修复子代理场景命中率抖动）**：**任何会进入历史或 tools 数组的字节都必须逐字节稳定**，否则每轮都会打断前缀缓存。① 工具结果的剪枝形态只有一处构造函数 `buildMicrocompactedToolOutputReference`（`message/tool-output-reference.ts`）——渲染期剪枝（`compaction/microcompact.ts`）与持久化标记（`message-to-model-messages.ts` 的 `time.compacted` 分支）**必须同源同字节**，禁止任一侧再带 `preview` / 图片数量等剪枝期上下文（历史缺陷：同一批消息在剪枝轮与下一轮被改写两次，连续两次打断缓存）；② 会进入 tools 数组或 system 前缀的**集合**必须确定性排序：扁平 MCP 工具（`mcp/mcp-flat-tool-defs.ts`：server 按 id、tool 按 name）、工作区动态工具（`tools/dynamic-tool-loader.ts` 按 name）；技能顺序是**优先级语义**（`skill-selection-routes` 测试锁定），不要按 id 重排；③ 工作区上下文会话级冻结（见上条）；④ **每轮微压缩默认关闭**（见下条）；⑤ **用户记忆块挂在最后一条用户消息**（`SyntheticRequestContext.memoryBlock`，随 synthetic block 持久化）而不是 dynamic system 尾段——记忆会被 `autoExtractMemoriesForRequest` / `memory_manage` 更新，挂 system 尾段时任何一次更新都会让「system + 全部历史」的缓存前缀整段失效。
- **微压缩（microcompact）默认关闭（2026-09-24，对齐参考库）**：参考库没有每轮剪枝——工具结果在**写时**截断（2000 行 / 50 KiB，全文落盘），历史保持**只追加**，只在接近上下文上限时整体压缩一次。本仓的 `compaction/microcompact.ts` 会每轮滑窗改写**历史靠前**的工具结果字节，导致每次剪枝都打断 prompt-cache 前缀（子代理等工具密集场景实测连续多轮 0 命中、命中率在 10%~90% 间抖动）。因此生产链路默认关闭：`stream-model-round.ts` 传 `{ enabled: isMicrocompactEnabled() }`，只有 `OPENAWORK_ENABLE_MICROCOMPACT=1` 才开启；机制单测仍直接给 `microcompactMessages(..., { enabled: true })` 覆盖。
...
- **OpenAI Chat 解析层对齐 opencode 参考库**（`packages/opencode-llm/src/protocols/openai-chat.ts`）：① **delta 字段宽容**——`OpenAIChatDelta` / `OpenAIChatChoice` / assistant 消息均用 `StructWithRest`，并显式声明 `refusal`（拒绝文本，按正文渲染）、`reasoning` / `reasoning_text` / `reasoning_details`（思维链字段变体）、`choice.usage`。**严格 Struct 会静默剥离未声明字段**：上游把可见内容放进这些字段时，客户端表现为「有思考、正文为空」（history 也会在回传时丢字段）；② **终态语义严格**——`mapFinishReason` 对未知值 / `error` / `network_error` 一律失败（不再折叠成 `'unknown'` 再被网关映射为 `end_turn`）；`ProtocolStream.onHalt` 与参考库一致**返回 Effect**（`route/protocol.ts`），`finishEvents` 在缺 `finish_reason` 且 `requireFinishReason`（默认 true，可用 `ModelCompatibility.requireFinishReason` 关闭）时**以 `incomplete-stream` 让整条流失败**（触发上层重试）。顶层 error 体仍以 `provider-error` 事件表达（网关依赖其 `context-overflow` 分类触发压缩），但会在 state 记 `providerFailed`，避免 `finishEvents` 叠加 incomplete-stream；网关侧 `provider-error` 同时标记 `doneEmitted`，避免再叠加误导性的 `STREAM_STALL`；③ **思维链是响应级通道**——`reasoning-0` 块保持打开，由 `finishEvents` 统一关闭，正文 / 拒绝文本出现不再关闭它；④ **SSE `retry:` 控制帧必须被忽略**——`sseFraming` 改为直接喂 `Sse.makeParser`（对齐参考库），不再走 `Sse.decode()`：后者把 `retry:` 当通道失败抛出，`catchTag(...Stream.empty)` 兜底会**静默截断该指令之后的全部事件**（长思考阶段出现 keep-alive 时表现为误报 `stream ended without finish_reason`）；⑤ **`[DONE]` 是终止哨兵**——openai-chat 用 `Framing.sseWithDone` 保留该帧并配 `stream.terminal`，读到即停止（不必等 HTTP body EOF；上游发完 `[DONE]` 不关连接时不再误判 STALL）；⑥ **`requireFinishReason=false` 可用**——`finishEvents` 在缺 `finish_reason` 时收尾未完成工具调用并**合成** `stop`/`tool-calls` 终态（此前不发 finish，外层 `requireTerminalEvent` 仍会失败），`providerFailed` 时则不追加任何事件避免「终态之后仍有事件」；⑦ **思维链元数据 / 历史回传对齐参考库**——`reasoning-delta` / `reasoning-end` 携带 `providerMetadata.openai.{reasoningField, reasoningDetails}`（字段名 + 累计结构化条目），`finishEvents` 用完整元数据关闭 `reasoning-0`（仅观测到结构化条目时补开一次）；请求侧按「配置 → 观测字段名 → native/`reasoning_content` 兜底」写回同名字段并原样回传 `reasoning_details`（`ModelCompatibility.requireReasoning` 可强制 DeepSeek 系始终带字段，`reasoningField` 命中保留字段会显式报错）。网关侧全链路透传：`stream-runner` 的 `thinking_end.providerMetadata` → `reasoning-blocks`（挂第一个块）→ `stream-model-round` 内容条目 → `message-v2-adapter`（`metadata.openai`，读路径 `v2ToV1Message` 同步回传）→ `message-to-model-messages`（`differentModel` 时剥离）→ `native-message-bridge`（还原为 `providerMetadata.openai`）。回归测试：`packages/opencode-llm/src/protocols/__tests__/openai-chat-delta-compat.test.ts`、`openai-chat-request-lowering.test.ts`、`sse-framing.test.ts`、`packages/opencode-llm/src/route/__tests__/openai-chat-stream-boundary.test.ts`、`services/agent-gateway/src/__tests__/session/reasoning-blocks.test.ts`、`services/agent-gateway/src/__tests__/message/message-v2-compat-regressions.test.ts`。
- **OpenAI Chat 请求侧 shaping + 元数据命名空间对齐**（`packages/opencode-llm/src/protocols/openai-chat.ts`、`route/client.ts`）：`ModelCompatibility` 新增 `maxTokensField` / `requireAssistantAfterTool` / `supportsStore` / `supportsUsageInStreaming` / `supportsStrictMode`，未配置时按 provider / baseURL 探测——原生 OpenAI → `max_completion_tokens`（o 系 / GPT-5 只认它），DeepSeek / Moonshot / Together / ZAI / Nvidia / Cerebras / Chutes / Cloudflare 白名单 → `max_tokens`；**移植版差异**：`custom` / `openai-compatible`（网关自建中转的默认类型）保守走 `max_tokens` 且不发 `store` / `strict`，避免严格网关 400。工具调用 ID 按模型归一化（Mistral 9 位字母数字 / Claude 字符集 / OpenAI 40 字符上限，assistant `tool_calls[].id` 与 `tool` 消息的 `tool_call_id` 同步）；`requireAssistantAfterTool`（Mistral 系自动推断）在工具结果后桥接 `Done.`。`Route` 新增 `providerMetadataKey`（优先级：补丁 → 路由显式值 → provider 字符串；**路由显式值粘性**，provider 覆盖不改写——网关思维链元数据固定读 `openai` 命名空间；参考库 openrouter 用 `openrouter`，移植版暂 pin 到 `openai`，待网关改为按路由读取后放开）。
- **Anthropic Messages thinking / beta 对齐**（`packages/opencode-llm/src/protocols/anthropic-messages.ts`）：`thinking` provider option 支持 `adaptive` / `disabled` / `enabled`（+`display: summarized|omitted`，`budgetTokens` / `budget_tokens` 双拼写）——此前只认 `enabled`，网关对部分模型下发的 adaptive 会被静默丢弃；官方端点走 `?beta=true` 并**始终**带 `interleaved-thinking-2025-05-14` beta（手工思考模型需要它在工具调用之间继续思考），context-management beta 按需追加；中转 / 兼容端点保持不主动下发 beta（沿用既有的 relay 安全策略）。
- **Anthropic Messages 文档 / PDF 支持**（同文件 `lowerMedia`）：媒体降级从「仅 base64 图片」扩展为对齐参考库——图片（base64 / HTTP URL / Files API `file_id`，+`transformations.oversized_image`）与 PDF 文档（base64 / URL / `file_id`）、`text/plain` 文本文档，并透传 `title` / `context` / `citations`（读 `part.metadata.anthropic.*` 或顶层同名键）；工具结果里的 PDF 附件同样降级为 `document` 内容块。`MediaPart` 补上 `cache` 字段以支持媒体块缓存断点。
- **Effect 升级到 4.0.0-rc.117 + 源码目录编译产物清理**：`packages/opencode-llm` 与 `services/agent-gateway` 从 `4.0.0-beta.83` 升到 `4.0.0-rc.117`。适配点：`Schema.TaggedErrorClass` → `Schema.TaggedError`（yieldable；rc 已移除前者，`LLMError` / `ToolFailure` 两个类）、`Config.redacted` → `Config.Redacted`、`Schedule.both` → `Schedule.max([...])`（rc 的合并语义：全部 schedule 都要 recur，取最大延迟）。**同时删除了 `packages/opencode-llm/src/**` 下 124 个被误提交的编译产物（`.js` / `.d.ts` / `.map`）**：它们会遮蔽同目录 `.ts`（相对 `.js` 导入被 Vite/Node 解析到陈旧 JS），升级后表现为 `Schema.TaggedErrorClass is not a function` 之类的假故障——以后不要往 `src/` 提交构建产物。
- **思维链元数据命名空间（网关按路由读取）**：`stream-runner` 用 `input.model.route.providerMetadataKey ?? String(input.model.provider)` 读写思维链字段名 / 结构化条目 / serviceTier；持久化链路（`reasoning-blocks` → `stream-model-round` → `message-v2-adapter`）把 key 一并落库（`metadata.providerMetadataKey` + `metadata[key]`，存量行缺省 `openai`），回传时由 `message-to-model-messages` / `native-message-bridge` 按同一 key 还原。**openrouter 因此改回参考库的 `openrouter` 命名空间**（不再 pin 到 `openai`）。
- **`prompt_cache_key`（OpenAI 会话级缓存亲和）**：`LLMRequest` 新增 `promptCacheKey`，`ModelCompatibility` 新增 `supportsPromptCacheKey`（默认关闭）；`ProviderShared.promptCacheKey` 按 64 字符截断并尊重 `cache: 'none'`。网关侧 `buildNativeModel` 只为原生 OpenAI chat 打开该开关，`stream-runner` 传 `sessionId` 作为 key；协议侧优先取请求级 `promptCacheKey`，回退到既有的 `providerOptions.openai.promptCacheKey` 生产者（Responses 路径历史行为不变）。
- **Anthropic 顶层 provider options 透传**：`output_config` / `cache_control` / `container` / `inference_geo` / `metadata` / `service_tier`（snake_case 与 camelCase 双拼写，snake 优先；Schema 校验，非法值边界报错）。
- **Anthropic 时序 effort 更新**：新增 `Message.effort({ effort, previous })` 标记 + `effort-updates.ts`（`effortUpdate` / `stripEffortUpdates` / `applyEffortUpdates` / `resolveEffortUpdates`）；路由级 `supportsEffortUpdates` 开关（Anthropic 按 Claude 版本推断：opus >= 5 / fable|mythos >= 5.1）。支持的路由把标记降级为原生 `output_config` system 消息、顶层 effort 冻结在首个标记的 `previous` 并请求 `mid-conversation-output-config-2026-07-01` beta；其余路由在编译期剥离标记；最后一个标记与请求 effort 不一致（回退/分叉历史）时剥离全部标记。
- **Anthropic thinking block binding**：`thinking.block_binding`（`prefix_mismatch_behavior`）透传；Claude >= 5.1 默认注入 `drop_block`（未配置 thinking 时补 adaptive），`disabled` 不注入；请求 `thinking-binding-controls-2026-08-01` beta。`ModelCompatibility.supportsThinkingBlockBinding` 可覆盖。
- **并行工具调用开关**：`ToolChoice.disableParallelToolUse` 新增；Anthropic 下发 `disable_parallel_tool_use`，Responses 推导 `parallel_tool_calls`（`providerOptions.openai.parallelToolCalls` 显式值优先）。
- **Gemini / Responses 补齐**：Gemini 支持 `safetySettings` / `serviceTier` / `thinkingConfig.thinkingLevel` 透传；Responses 支持 `providerOptions.openai.maxToolCalls` → `max_tool_calls`。
