...
- **渠道媒体能力（Telegram / Discord，2026-09-23 落地）**：入站图片走通用消费链路（`auto-reply.ts:400` 的 `buildInputImageParts` → `InputImageContent`，支持 base64 / imageUrl 双形态），两平台策略不同——**Telegram 必须下载转 base64**（文件 URL 携带 bot token 不可外泄；`channels/telegram-media.ts` 4MB 上限，getFile/下载失败只 warn 返回 `null`、消息按 `[User sent an image]` 占位符投递；轮询用 fire-and-forget `dispatchUpdate` 避免下载拖慢长轮询，且**不传** `pollAbort` 信号防竞态）；**Discord 直接映射 CDN 附件 URL**（签名 URL 约 24h、避免 base64 入库膨胀；`inbound-parsers/discord.ts` 4 张上限）。出站：Telegram `/sendPhoto` multipart（caption ≤1024 截断）、Discord `/channels/{id}/messages` multipart（`payload_json` + `files[0]`，content ≤2000 截断，**multipart 不得带 `Content-Type: application/json`**）。服务方法落地后 `PluginSendImage` / `replyImage` 自动可用（工具层按 `service.sendImage/replyImage` 做能力探测，`tools/channel-tools.ts:95-122`），无需改描述符。已知限制：relay/webhook 模式下的 Telegram 图片不下载（envelope 形态 `images` 仍被 `parseSimpleEnvelope` 支持）；非图片附件忽略；`sourceUrl` 在 Telegram/Discord 出站保留但不用（调用方先取 buffer）。
- **渠道入站媒体扩展点**：新增平台支持入站图片时，parser 只需产出 `ChannelMessage.images`（`ChannelImageAttachment`：`base64?` / `imageUrl?` / `mediaType` 必填 / `fileName?`），消费链路与 `PluginSendImage` 出站即自动打通；需要下载的平台参考 Telegram（`telegram-media.ts` + 服务层 enrich）或飞书（`feishu-gateway.ts:69-93`）模式。
