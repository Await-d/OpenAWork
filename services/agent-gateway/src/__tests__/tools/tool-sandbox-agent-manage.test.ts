/**
 * `agent_manage` 在 tool-sandbox 中的接线与权限门控回归。
 */
...
async function executeAgentTool(input: Record<string, unknown>, sessionId: string) {
...
describe('tool-sandbox · agent_manage 接线与门控', () => {
  beforeEach(() => {
  ...
  it('默认 ask 档位：create 产生 pending 审批，不写 agent_catalog', async () => {
    const result = await executeAgentTool(CREATE_INPUT, 'plain-session');
    ...
    expect(
      mocks.sqliteRunMock.mock.calls.some(
        ([query]) =>
          typeof query === 'string' &&
          query.includes('user_settings') &&
          query.includes('agent_catalog'),
      ),
    ).toBe(false);
  });

  it('只读 list 免审批：直接派发并返回内置 Agent', async () => {
    const result = await executeAgentTool({ action: 'list' }, 'plain-session');

    expect(result.isError).toBe(false);
    expect(result.pendingPermissionRequestId).toBeUndefined();
    const output = JSON.parse(String(result.output)) as { ok: boolean; count: number };
    expect(output.ok).toBe(true);
    expect(output.count).toBeGreaterThan(0);
  });

  it('yolo 档位：免审批派发并写入 agent_catalog', async () => {
    mocks.metadataJson = JSON.stringify({
      workingDirectory: TEST_WORKSPACE,
      permissionMode: 'yolo',
    });

    const result = await executeAgentTool(CREATE_INPUT, 'yolo-session');
  ...
  it('team 后台会话：权限层自动免审批，但工具内守卫仍拒绝', async () => {
    ...
    const result = await executeAgentTool({ action: 'list' }, 'team-session');
