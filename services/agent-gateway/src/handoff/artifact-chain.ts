...
import { setSubstate, SUBSTATES_C } from './substate-store.js';
import {
...
async function waitForClarificationAnswers(input: {
  ...
  while (Date.now() < deadline) {
    ...
    if (hasPendingCancelSignal(input.sessionId)) {
      // 把 cancel_signal 消费掉，然后抛错让上层 failHandoff
      consumePendingInboundMessage({ toSessionId: input.sessionId, loopIteration });
