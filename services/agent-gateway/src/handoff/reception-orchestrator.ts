...
import { publishHandoffEvent, publishTeamEvent } from './team-events-bus.js';
import { setSubstate, SUBSTATES_RECEPTION } from './substate-store.js';
...
export interface OrchestrateReceptionResult {
  ...
  triggered: boolean;
  reason?: 'feature-flag-disabled' | 'handoff-active' | 'no-llm-config' | 'llm-failed';
  handoffId?: string;
...
export async function orchestrateReceptionInput(
  ...
  if (!isHandoffModeEnabled()) {
    if (persistAck) {
      writeAck(input.userId, input.receptionSessionId, FALLBACK_ACK_FEATURE_DISABLED);
    }
    return { triggered: false, reason: 'feature-flag-disabled' };
  }
  ...
  // 推到 dispatching：前端 UI 能反馈"团队正在接管"
  setSubstate({
    sessionId: input.receptionSessionId,
    substate: SUBSTATES_RECEPTION.DISPATCHING,
    userId: input.userId,
    roleLayer: 'reception',
  });
...
const FALLBACK_ACK_FEATURE_DISABLED =
  '收到。当前团队编排开关未开启（`OPENAWORK_TEAM_HANDOFF_MODE`），仅记录消息。';
