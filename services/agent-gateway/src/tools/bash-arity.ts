/**
 * Bash Command Arity Prefix
 *
 * Ported from opencode's permission/arity.ts.
 * Extracts the "human-understandable command" from an argv token list.
 * Used for permission scoping — e.g. "git checkout main" → "git checkout".
 *
 * Rules:
 * - Longest matching prefix wins
 * - Flags never count as tokens, only subcommands
 * - If no entry matches, default arity is 1 (just the binary name)
 */
...
/**
 * Extract the command prefix tokens based on known arity.
 *
 * @example
 * bashArityPrefix(["git", "checkout", "main"]) // → ["git", "checkout"]
 * bashArityPrefix(["npm", "run", "dev"])        // → ["npm", "run", "dev"]
 * bashArityPrefix(["python", "script.py"])      // → ["python", "script.py"]
 */
export function bashArityPrefix(tokens: string[]): string[] {
...
/**
 * Tokenize a shell command string into words (basic shell-like splitting).
 ...
 *
 * @example
 * bashCommandScope("git checkout main") // → "git checkout *"
