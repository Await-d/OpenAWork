...
- 若为开放式（"改进一下"、"清理代码"），默认选择可逆、低风险、局部化的最小改善目标并写明假设；只有目标缺失、自相矛盾或会跨模块/破坏性影响时才追问具体目标
- 为所有阶段创建初始 todos
...
- 解析目标范围：
  ...
  - 用户说"整个仓库"或类似话术，默认按包/目录分批扫描并先产出候选，不先打断用户确认范围
    （仓库级扫描代价高，优先按包/按目录推进，删除前再按影响半径收口）
- 开放式表述（"清理代码"）默认先做只读候选发现，不删除；只有无法限定影响半径或会触及公共 API / 动态注册时才追问范围
