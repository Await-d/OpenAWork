...
export function withWorkflowRuntimeEvidenceArtifact(
  metadata: Record<string, unknown>,
  input: { readonly artifactId: string; readonly status: WorkflowRuntimeEvidenceStatus },
): Record<string, unknown> {
  const currentRefs = optionalStringArray(metadata['workflowRuntimeEvidenceArtifactRefs']);
  const artifactRefs = currentRefs.includes(input.artifactId)
    ? currentRefs
    : [...currentRefs, input.artifactId];

  return {
    ...metadata,
    workflowRuntimeEvidenceArtifactRefs: artifactRefs,
    workflowRuntimeEvidenceStatus: input.status,
  };
}

export function buildWorkflowRuntimeState(metadataJson: string | undefined): WorkflowRuntimeState {
