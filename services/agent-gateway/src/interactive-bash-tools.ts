  ...
  if (trackingContext && !outputIsError) {
    try {
      if (TMUX_SPAWN_SUBCOMMANDS.has(subcommand)) {
        ...
        const { registerTerminal } = await import('./session-terminal-registry.js');
        registerTerminal({
        ...
        const { markTerminalExited } = await import('./session-terminal-registry.js');
        if (subcommand === 'kill-server') {
          ...
          const { listSessionTerminals } = await import('./session-terminal-registry.js');
