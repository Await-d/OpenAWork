...
**状态**: 实现与复查修复完成，待提交
**开始日期**: 2026-09-15
**工作流文档**: [workflow/260915-文件提及索引架构.md](workflow/260915-文件提及索引架构.md)

**目标**: 修复 `@` 只能看到工作区根目录文件的问题（候选来自 `/workspace/tree?depth=2` + `slice(0,8)`，深层文件永远不可达），并按用户要求把索引移到网关缓存、使检索由用户输入驱动、前端不再全量加载。

**关键架构决策**:
- 网关新增递归扁平文件索引（BFS，无层数限制）+ 进程内缓存（TTL 15s / 上限 16 根 / 写路径显式失效），检索与排序放服务端（目录逐级 / 相关性排序）；删除零生产消费者的全量清单端点。
