...
- [2026-04-12] C-03 已冻结 Shared / Bridge 模型：shared session 只保留 bridge/import/detail 语义，不再驱动 Team 主导航；后续必须补 `POST/GET /team/workspaces/:teamWorkspaceId/imports` 与 `POST /chat/:personalSessionId/share` 作为显式入口。
- [2026-04-12] D-01 已冻结受影响文件矩阵：`routes/team.ts`、`routes/sessions.ts`、`session-shared-access.ts`、`packages/web-client/src/team.ts`、`packages/web-client/src/sessions.ts`、`use-team-collaboration.ts`、`team-runtime-reference-data.tsx` 等为单 owner 热点；实现阶段必须区分 must-touch / transitional / late-stage 文件层级。
- [2026-04-12] D-02 已冻结迁移顺序：必须按 **Stage 1 边界/contract → Stage 2 写入路径 → Stage 3 读模型 → Stage 4 bridge 显式化 → Stage 5 页面热点/遗留壳收口** 推进，禁止把边界切换、读模型切换和页面热点改造打包成同一阶段。
- [2026-04-12] D-03 已冻结旧投影过渡矩阵：`sessions` 留在 Personal 域，`sharedSessions` 降级到 bridge/import panel，`selectedSharedSessionId` / `workspacePath` 最终删除，`selectedSharedSession` / `runtimeTaskGroups` / `pendingPermissions` 等只允许先包进 Team 聚合层再暴露给 UI。
- [2026-04-12] E-01 已冻结实现恢复 gate：只有 A-D 全部冻结后，才允许从 Stage 1/2/3 恢复代码改造；办公室视图深化、interaction-agent 主流程与视觉热点改造仍需后置。
- [2026-04-12] E-02 已冻结真相源约束：`TeamWorkspace` 是 Team 一级真相源，所有 session/shared-session/workspacePath 只能作为桥接或投影输入，必须先包进 Team 聚合层后才能被 UI 消费。
