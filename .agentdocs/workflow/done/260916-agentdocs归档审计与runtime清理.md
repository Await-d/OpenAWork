...
- **后续更新（2026-09-16 同日）**：上一条中"仍在活跃"的 **5 个方案已按用户决定全部归档并直接删除文件**（`250109` / `250815` / `260704` / `260706` / `260814`），未做项统一标注「用户决定放弃」，登记见 `index.md` 的「2026-09-16 批量归档（用户决定）」区块。由此本文件第 100 行的保留理由（`260706` 对应方案"仍活跃"）已失效——其 `runtime/260706-fusion-layout-t1-s2-refactor/` 按 cleanup-policy 已可回收，但 `runtime/` 属 `.gitignore` 且删除不可从 git 恢复，故留待人工确认，与本文件 P4 口径一致。
