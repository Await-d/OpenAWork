...
- [x] T1. 抽象 Virtual MCP Provider Registry
  ...
  - 结果：已由 executor 完成并由 gate reviewer confirmed；证据见 `.omo/evidence/omo-mcp-adapter-integration/t1-virtual-provider-registry.txt` 与 `.omo/evidence/omo-mcp-adapter-integration-t1-virtual-provider-registry-gate-review.md`。
  - Commit：`feat(gateway): 抽象虚拟MCP提供者注册表`

- [x] T2. 新增 OMO Adapter Manifest 解析层
  ...
  - 结果：已由 executor 完成；首轮 gate 发现 Prettier/ESLint 静态门禁问题后已修复，第二轮 gate confirmed。注意 `omo-tool-manifest.ts` 处于 200-250 pure LOC warning band，后续新增逻辑前应优先拆分。
  - Commit：`feat(gateway): 新增OMO能力清单适配器`

- [x] T3. 将 OMO 能力映射为 MCP Catalog
  ...
  - 结果：已新增 inert `omo` virtual MCP provider 并通过 registry 接入；经补齐 code review/manual QA/dirty boundary 证据与 T6 typecheck 污染修复后，最终 gate confirmed。
...
- [x] T4. 收口执行层与 Hook 边界
  ...
  - 结果：已确认 flat MCP 与 legacy `mcp_call` 共享 permission category/storage/lookup；hook before/after/throw/unregistered 边界、OMO flat/legacy session scope 与 audit/permission observables 均经 gate confirmed。
  - Commit：`feat(gateway): 收口OMO工具执行与Hook边界`

- [x] T5. 完善 Settings MCP 管理面
  ...
  - 结果：已 confirmed。Settings MCP 管理面现在以 gateway runtime/settings 为同源数据，protected builtin/virtual/adapter rows 清洗 fake endpoint，shared-ui 真实 public boundary 有包内 barrel 测试，apps/web mock alias 已披露为 app contract，focus/desktop/mobile 视觉证据与 lockfile dependency boundary 均经 gate confirmed。
  - Commit：`feat(web): 完善内置与OMO MCP管理界面`

- [x] T6. 补齐 Team 与 Session MCP 授权策略
  ...
  - 结果：已补齐 catalog 与执行层双重授权；`mcp_call`、flat MCP 和 list 路径均传入 session MCP scope。经多轮 gate 修复执行层 hard deny、测试拆分、code review 与 dirty-worktree 证据后 confirmed。
  - Commit：`test(gateway): 补齐团队会话MCP授权策略`

- [x] T7. 编写 MCP/OMO 架构 ADR 与用户说明
  ...
  - 结果：已由 executor 完成并由第二轮 gate reviewer confirmed；首轮 gate 仅返回无证据 `REJECT`，已按无效门禁重派。
...
- [x] F1. Plan compliance audit：核对 T1-T9 是否满足 Scope IN/OUT，特别是不引入 `lazycodex-ai` 运行时依赖、不写 Codex 用户目录、不绕过 sandbox。证据：`.omo/evidence/omo-mcp-adapter-integration/f1-plan-compliance.md`。
- [x] F2. Code quality review：检查 TypeScript 严格类型、NodeNext `.js` 导入、无 `any`/TS suppressions、文件体积、无空 catch。证据：`.omo/evidence/omo-mcp-adapter-integration/f2-code-quality.md`。
- [x] F3. Security/permission review：确认 hook、OMO adapter、virtual MCP、team allowlist 都不能扩大权限或绕过审计。证据：`.omo/evidence/omo-mcp-adapter-integration/f3-security-permission.md`。
- [x] F4. Real manual QA：设置页 + chat + team 三个真实表面均完成 smoke。证据：`.omo/evidence/omo-mcp-adapter-integration/f4-manual-qa/README.md`。
