...
- [x] T-14: 拾取与上下文交付的测试已完成；**375/768/1280 的机械走查已由真实浏览器完成**（`ui-walkthrough` 连续 3 次 9/9：帧尺寸精确 375×812 / 768×1024 / 1280×800 + 像素命中目标页；`acceptance-ui` 8/8 含同一组预设）。**仅剩人工观感复核**（机器无法替代"好不好看"）
...
- [x] T-15: ✅ **2026-09-16 完成**：Fusion 右侧停靠面板新增第 4 个 tab「浏览器预览」（`SidePanelTabId` 扩 `'browser'` + `PANEL_TAB_ORDER` + 键盘循环）；新增自持状态的 `panels/FusionBrowserTab.tsx`（119 行）；`FusionSessionSidePanel` 加分支。**关键：单一浏览器互斥**——`FusionBrowserTab` 挂载即 `setBrowserPreviewSurface('dock')`、卸载归还 `'editor'`，`EditorBrowserWorkspace` 据同一标记 `!browserHostedByDock` 拒绝挂载第二份 `BuiltInBrowser`（否则两实例各自建网关实时会话，controller 选举与 ack 额度互相抢占）。**真实浏览器验收**：空态→填址→打开→实时画面渲染 ✅、互斥成立（两 surface 同在时全应用仅一个实例）✅、对照组正常 ✅。（原先担心的"宿主被并行重构"最终未造成不可调和冲突）
...
- [x] T-21: ✅ **2026-09-16 补充实施并验证**：包层 `domTree()`（深度钳制 + 2000 节点上限 + `truncated`）/ `accessibilitySnapshot()`（Playwright 引擎无关 API）+ 契约新增 `dom`/`a11y` 通道与 `dom.tree`/`a11y.tree`/`node.styles` 控制动作 + 网关回包；UI 为 `BrowserInspectorPanel.tsx`（+`browser-inspector-{model,tree,detail,chrome,tokens}`）。**真实浏览器验收 13/13**（DOM/无障碍树、节点详情、**451 条样式可搜索过滤 + 「显示全部」增行**、mixed/disabled 状态芯片、**canvas 客观 WCAG AA 对比度测量**）；真实网关端到端 6/6（`dom.tree` 属性扁平化并找到探针元素 / `a11y.tree` 含 `{role:"button",name:"提交订单"}` / `node.styles` 451 条 / 浅深度 `truncated:true`）
- [x] T-22: ✅ **2026-09-16 补充实施并验证**：包层捕获 CDP `Runtime.consoleAPICalled` 栈并与 Playwright 文本**有序关联**（避免重复事件）+ `source-map-resolver.ts`（**零新增依赖**，用 Node 24 内置 `module.SourceMap`；支持内联/外部 `sourceMappingURL`、相对路径解析、缓存有界、超时容错、绝不抛出）+ 契约加法扩展 + 网关条件转发；UI 在控制台渲染可展开堆栈。**端到端 3/3**（原始栈 `bundle.js:0:8` → **映射成功 `original.ts`**）；**UI 9/9**（0-based→1-based、未映射帧明确标记不冒充源码、每帧可复制）
- [x] T-23: ✅ **2026-09-16 补充完成**：合并版验收脚本 `acceptance-backend` **22/22**、`acceptance-ui` **8/8**、`ui-inspector-verify` 13/13、`ui-stack-verify` 9/9、`ui-sourcemap-verify` 3/3 → **合计 55/55**；测试合计 **1092**；`apps/web` 生产构建与各包 typecheck 全 exit 0。另有 8 个"只有在真实浏览器/真实网关才显形"的缺陷被逐一根因并修复（见 `index.md` 的缺陷账本）

### Phase P3-remainder：收口

- [x] T-24: ✅ **2026-09-16 补充完成**：能力协商审计与收口（`useEngineCapability` 单一事实来源、reason token → 可操作中文提示全覆盖）+ 新增运行手册 **`docs/browser-preview.md`（298 行）**：能力矩阵（含 **Chromium-only screencast**、Tauri 原生 webview 只能展示）、浏览器解析顺序（受管目录 → 系统 Chrome → Edge → 引导安装）与 `PLAYWRIGHT_BROWSERS_PATH` 语义、5 个 reason token 的提示与修法、协议摘要（信封/通道/**单帧信用 + 4s 看门狗**）、已知限制、5 个真实故障排查

## Notes

- **⏸️ 本轮挂起决定（2026-09-15，用户指示）→ ✅ 已于 2026-09-16 全部实施**：2026-09-15 用户曾指示「完成其他的方案，不执行这个方案」，故 T-21 / T-22 / T-23 / T-24 当时挂起、本文档随之归档。**2026-09-16 用户改为要求全部完成**，四项均已实施并逐项验证（见上方 T-21/T-22/T-23/T-24 条目），另有 T-15 可停靠面板、T-18 保存自动刷新、in-app 浏览器引导下载器三项后补完成。此段保留作为决策沿革记录，**不代表最终状态**。挂起时的技术依据（实测，非推测）：执行前探测发现**存在并发写入会话**正在修改本方案的前端文件域 — `apps/web/src/components/chat/misc/browser/BrowserConsolePanel.tsx`(22:15)、`NetworkWaterfall.tsx`(22:21)、`network-har.ts`、`browser-console-types.ts`、`live-console-bridge.ts`、`BuiltInBrowser.tsx`、`browser-live-harness.tsx`(22:27)；同时机器上有 3 个长驻 `opencode` 进程 + 既有 gateway dev(tsx watch) + vite --port 5199。后续实践证伪了"必须挂起"的结论——**正确应对是分阶段施工 + 逐处复核 + 独立验证**（全程未发生不可调和的冲突），且该争用期恰好暴露了 8 个只有真实环境才能发现的缺陷。
