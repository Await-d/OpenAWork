...
### 0.159 失败派发的用户出口 + 规划调查的 JSON 纠正重试（防「不可恢复终态」永久挂起且无自救路径）（2026-09-21 续）

- 问题：两条互相叠加的缺口。(1) **终态无出口**：`handoff/capability/planning-failure.ts` 给所有 `PlanningFailure` 强制追加 `；需要用户介入`，命中 `handoff/store/handoff-store.ts` 的 `UNRECOVERABLE_FAILED_HANDOFF_REASON_SUBSTRINGS` → `isRecoverableFailedHandoff()` 恒 false → `retryFailedHandoff` 与 `team/team-runtime-remediation-policy.ts` 的自动补救全部跳过；`handoff/runner/watcher.ts` 又把 `planning-generation-failed:` 前缀**显式排除**出 PM1→PM2 降级链，并把接待层 substate 推回 `idle`；而 `cancelHandoff` 明确拒绝 `failed`。于是这条失败**既不能重试、不能降级、也不能取消**——前端 `ErrorDiagnosticsPanel` 由 failed handoff 列表驱动，会永久显示 `[PM1] planning-generation-failed: …` 且无任何可点动作。更糟的是关闭入口虽已按角色放宽，却被 `canManageSelectedRuntimeTree`（要求 `selectedTeam.status !== 'failed'`）门控，而失败 handoff 恰好把团队状态推成 `failed`，形成自锁。(2) **触发面过脆**：`handoff/runner/planning-investigation.ts` 对模型回复做 JSON 解析，**一次畸形输出即抛终态 `PlanningFailure`**，且非法原文被直接丢弃——工作区/会话里没有任何「拦截记录」可查，用户只看到一句笼统结论。
- 加固：(a) 调查动作与收尾两处解析改为 `requestJson()`：解析失败时把「上次解析错误 + 上次原文摘录」回灌给模型要求重发合法 JSON（`JSON_REPAIR_ATTEMPTS = 2`，最多 1+2 次；预算有界，既有精确调用次数断言不变）；(b) 失败消息携带原文摘录（`summarizeRawOutput`：折叠空白、trim、≤300 字符、空响应回落 `(空响应)`），随 `failure_reason` 落库，并可经会话消息与诊断面板 tooltip 查阅；(c) 新增 `dismissFailedHandoff` 与 `POST /team/handoffs/:handoffId/dismiss`：把不可自动恢复的失败收敛为 `cancelled`（语义与判定登记在 `adr-turn-rollback-hard-delete.md` 附节，**不删除任何审计行**），并以 `sqliteRunWithChanges` 的 `changes > 0` 防并发误发 `handoff.cancelled`；(d) 运行时投影下发 `dismissableFailure`（与 `recoverableFailure` 同款单一判定源），前端据此渲染关闭入口，并把失败动作的权限判定从 `canManageSelectedRuntimeTree` 解耦为不含运行状态的 `canActOnRuntimeFailures`，同时补齐 classic 工作台入口、5 条上限下的批量关闭、会话级兜底动作与失败 toast。
- 与既有韧性的关系：与 §0.1–§0.158 那批「上游畸形/超时不得拖垮主链路」同族，这次补的是「**终态失败必须有用户出口**」这一维度——此前加固都在保证「不卡在 running」，这里暴露的是「卡在了 failed 且无人能清」。
- 回归证据：`__tests__/handoff/planning-investigation.test.ts` 新增 3 用例（首轮非 JSON 后修复重试成功 / 持续非 JSON 失败且消息含原文摘录 / 收尾修复重试）；`__tests__/handoff/handoff-store.test.ts` 与 `__tests__/team/team-handoffs-routes.test.ts` 覆盖 dismiss 的可恢复拒绝、孤儿 executor 允许、PM2 仍在裁决时 409、重复关闭返回 false；`ErrorDiagnosticsPanel.test.tsx` 覆盖 `recoverableFailure` / `dismissableFailure` 门控、5 条上限下的批量关闭与已 handled 项不再入榜。gateway handoff 套件与 web 面板套件全绿。

### 0.160 规划失败按处置分级 + 失败重试预算（防「瞬时失败被当成需人工介入」与「可恢复但注定失败者无限重试」）（2026-09-21 续）

- 问题：两个方向相反的缺陷。(1) **过度终态**：`capability/planning-failure.ts` 给**所有** `PlanningFailure` 无条件追加 `；需要用户介入`，于是模型一次输出畸形、或未交付完整正文这类**瞬时失败**也被打成 `requires-user` 终态 —— 命中 `UNRECOVERABLE_FAILED_HANDOFF_REASON_SUBSTRINGS` 后 `retryFailedHandoff` 恒拒绝，用户既不能重试、也无法（在 §0.159 之前）关闭。(2) **无限重试 / 永不收敛**：`retryFailedHandoff` 没有重试上限，且它把 `failed` 重置为 `pending` 时**不递增 `retry_count`**；`heartbeat-timeout-max-retry-exceeded`（心跳回收路径在 `retry_count >= maxRetry` 时写入，其命名本身即声明「已达上限」）却**没有被判为不可恢复**，于是它每次被重试后立刻再次超时、再次判上限，形成 `pending → stale → failed` 的无限空转，持续消耗额度且永远无法关闭。同时 `retry_count` **不能**直接挪用为失败重试预算——它被 `nextPlanningRound(payload, retryCount)` 消费，进而喂给 PM1 的自动规划返工上限（`pm1-runner.ts` `已达到自动规划返工上限`）与 `pm2-runner.ts`，复用会让两个上限互相污染。
- 加固：(a) `PlanningFailure(reason, disposition)`，`disposition: 'requires-user' | 'recoverable'`，**默认 `'requires-user'` 保持既有消息逐字不变**；仅四个瞬时点显式标 `'recoverable'`（调查 JSON 无效 / 调查动作不符合只读协议 / 收尾协议不完整 / 模型未交付完整正文），其余（无进展、无文件证据、证据不足、返工上限、预算耗尽、规划校验失败、grill 耗尽等）仍为 `requires-user`。(b) 新增独立计数列 `handoff_records.failed_retry_count`（`ensureColumn` 附加，同步进 `handoff_records` 重建的显式列清单）与 `MAX_FAILED_HANDOFF_RETRY_COUNT = 3`：`retryFailedHandoff` 在预算耗尽时拒绝重试、成功重试时递增该列，**`retry_count` 语义完全不动**。(c) `isDismissableFailedHandoff` 由「拒绝一切可恢复失败」改为「拒绝**仍可重试**者」（原因可恢复 **且** 预算未耗尽），因此预算耗尽后自动转为可关闭——任何终态失败都仍有用户出口。(d) `heartbeat-timeout-max-retry-exceeded` 归入不可恢复前缀，止住无限空转并使其可关闭。(e) watcher 的 PM1 规划失败文案按可重试性分流（可重试者不再谎称「已停止自动重试」，改为指引「一键重试」）。
- 与既有韧性的关系：与 §0.159（终态必须有用户出口）互补——§0.159 补「出口」，本项补「分类正确性 + 重试有界」；与 §0.40/§0.54–§0.57（只增结构必须有界）同属「任何可被外部反复触发的循环都必须有界」，这次补的是「失败重试循环」这一维度。
- 回归证据：`__tests__/handoff/planning-failure.test.ts` 断言默认为 `requires-user` 尾注、`'recoverable'` 无尾注且 `isRecoverableFailedHandoffReason` 分别为 false/true；`planning-investigation.test.ts` 断言持续非 JSON 的抛错不含 `需要用户介入`；`handoff-store.test.ts` 新增 `失败重试预算` 用例（恰好重试 3 次且计数递增、第 4 次拒绝、预算未耗尽时不可关闭、耗尽后可关闭且 `retry_count` 保持 0、heartbeat 上限耗尽不可恢复且可关闭）；`team-runtime-routes.test.ts` 断言同一失败「预算剩余 → `dismissableFailure:false`、预算耗尽 → `true`」；`handoff-watcher.test.ts` 新增可重试分流文案用例。`__tests__/handoff`（39 文件）+ `__tests__/team`（34 文件）共 761 用例全绿。
