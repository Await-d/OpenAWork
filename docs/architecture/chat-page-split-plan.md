...
> **状态**：域 D / A / C / B / E **均已抽出**（B = `conversation/render/use-chat-streaming.ts`、E = `hooks/use-chat-retry-and-edit.ts`，两者均已被 `ChatPage.tsx` import 并调用）；ChatPage 现保留组装层与跨域协调外壳
> **创建时间**：2026-05-21
> **最近更新**：2026-09-21（状态复核）
> **触发**：`apps/web/src/pages/chat-page/ChatPage.tsx` 当前 **7347 行**（2026-09-21 实测），仍超过 AGENTS.md 1500 行硬上限 **4.9 倍**
> **关联**：React 19 改造收尾后的 P2 项

> ⚠️ **2026-09-21 复核更正（保留沿革，不改下文历史快照）**：
> ① 文末矩阵中 B / E 两行标注的「待排期」**已过期**——两域 hook 早已落地并接线（见上）。
> ② 计划期快照（`6052 行` / `5626 行`；hook 密度 24 / 28 / 25 / 22 / 15）**已失效**，实测为 **7347 行 / 35 `useState` / 36 `useEffect` / 36 `useCallback` / 27 `useMemo` / 26 `useRef`**。
> ③ 因此「剩余体积的根因是缺 hook 抽取（密度在域 B）」这一判断**已不成立**：五个域都已抽出，而文件实测为 7347 行、较计划期 5626 行 **+1721 行**（期间有功能叠加）。**剩余真实问题是「组装层 + 跨域协调 + 主组件仍集中 35 个 `useState`」**——需要一个以「组装层瘦身」为目标的新方案，而非继续沿用本计划的域 D→A→C→B→E 顺序（该顺序已走完）。
...
| B · 流式            | 2 天       | 1.5 天   | 1 天     | 4.5 天      | ✅ 已完成（2026-09-21 复核：`conversation/render/use-chat-streaming.ts`） |
| E · 重试/编辑       | 0.5 天     | 0.5 天   | 0.5 天   | 1.5 天      | ✅ 已完成（2026-09-21 复核：`hooks/use-chat-retry-and-edit.ts`） |
| **合计**            | **4.5 天** | **4 天** | **3 天** | **11.5 天** | **5/5（域抽取全部完成；文件仍 7347 行超限，另需「组装层瘦身」新方案）** |
...
**下一步（2026-09-21 复核更新）**：域 D → A → C → B → E 的 5 个 PR **已全部落地**（见文末矩阵 5/5）；本计划的域抽取阶段**已结束**。文件仍 7347 行（超硬上限 4.9 倍），后续需另立一份以「**组装层瘦身 + 主组件状态下移**」为目标的新方案（目标：把 35 个 `useState` 按域下沉、跨域协调收敛为少量组合 hook），不再沿用本计划的任务序列。
