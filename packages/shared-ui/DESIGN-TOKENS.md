...
> **线是例外，不是默认。** 结构靠 背景层级 + 间距 表达，不靠 1px 描边。
> 全站 1700+ 处描边都挂在这 5 个变量上，改这 5 个数就能整体控制界面的线量。
> **8 个主题共用下表同一组值**（颜色与 alpha 都统一）：暗色统一用冷灰 `hsl(215 …)`，
> 亮色统一用深蓝灰 `rgb(15 23 60 …)`。
> 为什么颜色也要统一：同一 alpha 下白色描边和冷灰描边的感知亮度差能差一倍
> （实测 dL\* 6.6 vs 2.8），只统一 alpha 的话换个主题线的重量就会突变。
> 主题的个性由 `--bg-*` / `--accent-*` 承担，描边不再掺主题色。

| 级别      | 暗色                      | 亮色                    | 用途                                           |
| --------- | ------------------------- | ----------------------- | ---------------------------------------------- |
| invisible | `hsl(215 20% 50% / 0)`    | `rgb(15 23 60 / 0)`     | 已废弃，恒全透明，仅为兼容保留                 |
| subtle    | `hsl(215 20% 50% / 0.03)` | `rgb(15 23 60 / 0.034)` | 同色系相邻块的边界，几乎不可见                 |
| default   | `hsl(215 18% 50% / 0.06)` | `rgb(15 23 60 / 0.065)` | 容器 / 卡片 / 列表行的主边界                   |
| emphasis  | `hsl(215 16% 55% / 0.13)` | `rgb(15 23 60 / 0.14)`  | 需要看得见的功能线：控件边界、拖拽柄、hover 态 |
| strong    | `hsl(215 14% 60% / 0.20)` | `rgb(15 23 60 / 0.22)`  | 选中 / 焦点 / 必须明确可点的控件               |

判定规则：

- 装饰性描边（卡片、面板、列表行、分组、分区）→ `subtle` / `default`。
- 只有「不加线就认不出这是个可交互控件」时才用 `emphasis` / `strong`。
- **以下三类不跟随上面的描边刻度**，它们不是装饰，淡到看不见等于坏掉，走独立 token：
  - `--scrollbar-thumb` / `--scrollbar-thumb-hover` —— 滚动条拇指
  - `--switch-track-off` —— 开关关闭态轨道
  - 拖拽柄用 `--border-emphasis`（默认 `opacity: 0`，hover 才显形，本就按需出现）
