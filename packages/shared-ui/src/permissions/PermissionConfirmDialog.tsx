...
const RISK_COLOR: Record<string, string> = { true: color.danger, false: 'var(--fg-muted, #7b8a9e)' };
...
const s: Record<string, CSSProperties> = {
  ...
  title: { margin: 0, fontSize: 14, fontWeight: 700, color: 'var(--fg-default, #c8d1e0)' },
  ...
  permScope: { color: 'var(--fg-default, #c8d1e0)', flex: 1, fontSize: 12 },
...
export function PermissionConfirmDialog({
  ...
  const trustColor = TRUST_COLOR[trustLevel] ?? 'var(--fg-muted, #7b8a9e)';
  ...
  return (
    <div style={s.overlay}>
      ...
      >
        ...
        <div style={s.footer}>
          <button type="button" style={actionBtn('var(--fg-muted, #7b8a9e)')} onClick={onCancel}>
            取消
          </button>
          <button
            type="button"
            style={actionBtn(color.fgOnAccent, 'var(--accent, #5cd4c0)')}
            onClick={onConfirm}
          >
            确认安装
          </button>
