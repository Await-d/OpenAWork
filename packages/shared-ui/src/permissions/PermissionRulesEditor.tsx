...
const ACTION_META: Record<PermissionAction, { label: string; color: string; icon: string }> = {
  allow: { label: '允许', color: 'var(--success, #3dd49a)', icon: '✓' },
  deny: { label: '禁止', color: 'var(--danger, #f06b7e)', icon: '✕' },
  ask: { label: '询问', color: 'var(--warning, #f0b429)', icon: '?' },
};
        ...
        return (
          <button
            ...
            style={{
              ...
              color: isActive ? meta.color : 'var(--fg-muted, #7b8a9e)',
    ...
    >
      ...
      >
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 10, color: 'var(--fg-muted, #7b8a9e)', marginTop: 2 }}>
        ...
        {category.supportsPatterns && (
          <button
            ...
            style={{
              ...
              padding: '2px 6px',
              fontSize: 9,
              borderColor: expanded ? 'var(--accent, #5cd4c0)' : undefined,
              color: expanded ? 'var(--accent, #5cd4c0)' : undefined,
            }}
          >
            {expanded ? '收起' : '模式'}
          </button>
  ...
  return (
    <div style={{ ...CARD, ...style }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        {saving && (
          <span style={{ fontSize: 10, color: 'var(--fg-muted, #7b8a9e)' }}>保存中…</span>
        )}
      </div>
