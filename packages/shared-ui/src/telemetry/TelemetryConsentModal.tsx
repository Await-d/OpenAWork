  ...
  return (
    <div style={overlayStyle} role="dialog" aria-modal="true" aria-labelledby="telemetry-title">
      <div style={modalStyle}>
        ...
        >
          ...
          {DATA_POINTS.map((point) => (
            ...
            >
              ...
              <span style={{ fontSize: 12, color: 'var(--fg-default, #c8d1e0)', lineHeight: 1.4 }}>
                {point}
              </span>
            </div>
          ))}
        ...
        <p
          style={{ margin: 0, fontSize: 12, color: 'var(--fg-muted, #7b8a9e)', lineHeight: 1.5 }}
        >
          你可以随时在偏好设置中更改此选项。数据以匿名方式发送，绝不会与第三方共享。
        </p>

        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          <button type="button" onClick={onDecline} style={btnStyle('var(--fg-muted, #7b8a9e)')}>
            不了，谢谢
          </button>
          <button type="button" onClick={onAccept} style={btnStyle('var(--accent, #5cd4c0)')}>
            启用遥测
          </button>
        </div>
      </div>
