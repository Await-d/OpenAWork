...
export function createPermissionsClient(gatewayUrl: string): PermissionsClient {
  return {
    async listPending(token, sessionId, options) {
      ...
      if (!res.ok) {
        throw new HttpError(`Failed to list pending permissions: ${res.status}`, res.status);
      }
    ...
    async createRequest(token, sessionId, payload) {
      ...
      if (!res.ok) {
        throw new HttpError(`Failed to create permission request: ${res.status}`, res.status);
      }
    ...
    async reply(token, sessionId, payload) {
      ...
      if (!res.ok) {
        throw new HttpError(`Failed to reply permission request: ${res.status}`, res.status);
      }
    },
  };
}
