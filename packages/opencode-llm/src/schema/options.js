...
export const mergeJsonRecords = (...items) => {
    ...
    if (defined.length === 0)
        return undefined;
    if (defined.length === 1 && defined[0] && Object.values(defined[0]).every((value) => value !== undefined))
        return defined[0];
    const result = {};
...
export const AnthropicProviderOptions = Schema.StructWithRest(Schema.Struct({
    contextManagement: Schema.optional(AnthropicContextManagement),
}), [Schema.Record(Schema.String, Schema.Unknown)]);
export const ProviderOptions = Schema.StructWithRest(Schema.Struct({
