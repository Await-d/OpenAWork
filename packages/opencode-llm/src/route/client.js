...
const endpointBaseURL = (endpoint) =>
  typeof endpoint.baseURL === 'string' ? endpoint.baseURL : undefined;
const mergeHeaders = (...items) => {
  const entries = items.flatMap((item) =>
    item === undefined ? [] : Object.entries(item).filter((entry) => entry[1] !== undefined),
  );
  if (entries.length === 0) return undefined;
  return Object.fromEntries(entries);
};
export const generationOptions = (input) =>
...
function makeFromTransport(input) {
  ...
  const build = (routeInput) => {
    Endpoint.validate(routeInput.endpoint);
    const route = {
      id: routeInput.id,
      provider:
...
const generateWith = (stream) =>
  Effect.fn('LLM.generate')(function* (request) {
...
export const streamRequest = (request) =>
  Stream.unwrap(
    Effect.gen(function* () {
      return (yield* Service).stream(request);
    }),
