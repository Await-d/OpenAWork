...
const bodyWithOverlay = (body, request, encodeBody) => Effect.gen(function* () {
    ...
    if (ProviderShared.isRecord(body)) {
        const overlay = ProviderShared.supportsAnthropicContextManagement(request)
            ? request.http.body
            : Object.fromEntries(Object.entries(request.http.body).filter(([key]) => key !== "context_management" && !key.startsWith("clear_")));
...
const stripContextManagementBeta = (value) => value
    .split(",")
...
const requestHeaders = (input) => {
    ...
    const beta = routeBeta === undefined
        ? customBeta
    ...
    if (ProviderShared.supportsAnthropicContextManagement(input.request))
        return merged;
    return Object.fromEntries(Object.entries(merged).flatMap(([key, value]) => {
