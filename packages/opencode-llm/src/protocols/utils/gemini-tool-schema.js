...
const emptyObjectSchema = (schema) =>
  schema.type === 'object' &&
  (!isRecord(schema.properties) || Object.keys(schema.properties).length === 0) &&
  !schema.additionalProperties;
// `nested` keeps empty object schemas one level down: Gemini needs a concrete
// `type` for property/array nodes, so only the root may collapse to undefined.
const projectNode = (schema, nested = false) => {
  ...
  // JSON Schema expresses `T | null` two ways: `type: ["T", "null"]` and
  // `anyOf: [{...T}, {type: "null"}]`. Both must lower to a Gemini `nullable`
  // node; a lone non-null branch can be flattened into the node itself.
  const hasNullAnyOf = anyOf?.some((item) => isRecord(item) && item.type === 'null') ?? false;
  const anyOfTypes = hasNullAnyOf
    ? anyOf?.filter((item) => !isRecord(item) || item.type !== 'null')
    : anyOf;
  const flattenedAnyOf =
    hasNullAnyOf && anyOfTypes?.length === 1 ? projectNode(anyOfTypes[0], true) : undefined;
  const result = Object.fromEntries(
    [
      ...
      [
        'nullable',
        (Array.isArray(schema.type) && schema.type.includes('null') && types && types.length > 0) ||
        hasNullAnyOf
          ? true
          : undefined,
      ],
      ...
      [
        ...
        Array.isArray(schema.allOf)
          ? schema.allOf.map((item) => projectNode(item, true))
          : undefined,
      ],
      [
        'anyOf',
      ...
      [
        ...
        Array.isArray(schema.oneOf)
          ? schema.oneOf.map((item) => projectNode(item, true))
          : undefined,
      ],
...
//# sourceMappingURL=gemini-tool-schema.js.map
