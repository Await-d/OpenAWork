...
describe('role taxonomy', () => {
  ...
  it('given oracle reference metadata, when formatting canonical role, then it returns researcher/architect', () => {
    const oracle = REFERENCE_AGENT_ROLE_METADATA['oracle'];

    expect(oracle).toBeDefined();
    expect(formatCanonicalRole(oracle!.canonicalRole)).toBe('researcher/architect');
  });
});
