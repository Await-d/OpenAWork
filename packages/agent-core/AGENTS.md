...
- **Provider 类型**：`ProviderType` 联合类型——`anthropic | openai | deepseek | gemini | ollama | openrouter | qwen | moonshot | mimo | mistral | zhipu | doubao | groq | siliconflow | azure | xai | minimax | baichuan | hunyuan | qianfan | opencode-go | custom`。
- **Provider Catalog（平台单一事实来源）**：`src/provider/catalog.ts` 的 `PROVIDER_CATALOG` 是所有平台信息的唯一来源——内置预设(`presets.ts` 由它派生)、API Key 环境变量、上游 baseUrl/协议变体、thinking 下发风格、host→type 推断、别名归一、前端 UI 元数据(logo/显示名/字形)全部从这里产生。各平台的默认模型清单在 `src/provider/catalog-models.ts` 的 `CATALOG_DEFAULT_MODELS`（按 type 建键的 `Record`，穷尽约束——漏平台会编译失败），由 catalog 条目引用；拆出该文件是为了守住单文件 1500 行上限。**新增一个平台只需四步**：① 在 `types.ts` 的 `ProviderType` 加一个成员；② 在 `catalog-models.ts` 的 `CATALOG_DEFAULT_MODELS` 加该平台的模型数组；③ 在 `catalog.ts` 的 `PROVIDER_CATALOG` 加一个条目并引用它；④(可选)在 `apps/web/public/` 放 `logo-<type>.svg`。网关枚举/thinking 分支、前端选择器/设置页/团队模板下拉都会自动生效，无需再改其它文件。thinking 复用已有「风格」(`ProviderThinkingStyle`)时，连网关 `provider-options.ts` 都不必改。**例外——中转/聚合型平台**（同一 baseUrl 下按模型混用多种上游协议，或转售一手平台模型的渠道，如 `opencode-go`/`siliconflow`）：还需一个 `resolve.protocol` provider plugin（见 `services/agent-gateway/src/provider/plugins/`）来按模型决定协议，并让 `resolveThinkingStyle` / `catalogModelSupportsThinking` / `canConfigureThinkingForModel` 走「按 modelId 反推真实厂商」分支。
- **权限主链**：统一位于 `src/permission/`；历史 `src/permissions/` 兼容层已退役，不再作为可用目录。
- **会话权限阶梯**：`src/permission/session-permission-mode.ts` 定义 `ask`（默认）/`auto-edit`（文件编辑与写入自动放行）/`yolo`（跳过询问，但显式 `deny` 始终优先）三档。唯一解析入口 `resolveSessionPermissionMode`（:20）：合法 `permissionMode` 优先，否则回退 `yoloMode === true ? 'yolo' : 'ask'`（:23-27）。`permissionMode` 是规范键，布尔 `yoloMode` 只是它的派生投影。
- **auto-edit 范围常量**：`AUTO_EDIT_PERMISSION_CATEGORIES`（`edit`/`write`，:38）与 `AUTO_EDIT_EXCLUDED_TOOLS`（`workspace_review_revert`，:44）。中档位覆盖全部 `edit`+`write` 类工具（含 `apply_patch` 的删除/移动与 `ast_grep_replace` 的批量重写），唯一豁免 `workspace_review_revert`（丢弃评审状态而非编辑内容）；需要收紧时靠显式 `deny` 规则（deny 始终优先于档位快捷分支）。
- **已知缺口**：`tool-category-map.ts` 尚未把任何 notebook 工具映射到权限类别，未来的 notebook 编辑工具在 `auto-edit` 下仍会询问，直到显式映射到 `edit`。
