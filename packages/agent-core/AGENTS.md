...
- **auto-edit 范围常量**：`AUTO_EDIT_PERMISSION_CATEGORIES`（`edit`/`write`，:38）与 `AUTO_EDIT_EXCLUDED_TOOLS`（`workspace_review_revert`，:44）。中档位覆盖全部 `edit`+`write` 类工具（含 `patch` 的删除/移动与 `ast_grep_replace` 的批量重写），唯一豁免 `workspace_review_revert`（丢弃评审状态而非编辑内容）；需要收紧时靠显式 `deny` 规则（deny 始终优先于档位快捷分支）。
