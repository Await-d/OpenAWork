...
export function BuddyAgentBindingPanel({
   ...
   * 实时校验「语速覆盖」输入。
   *
   * 留空表示「继承全局」是允许状态；填了非数字或超出 0.5–2.0 即标错。
   * 这里返回 `null | string`，组件用它驱动 aria-invalid 与错误提示，并
   * 让保存按钮在有错时 disabled，避免用户点了才发现。
   */
  const voiceRateError = useMemo(() => {
    const trimmed = voiceRateInput.trim();
    if (trimmed.length === 0) return null;
    const parsed = Number(trimmed);
    if (!Number.isFinite(parsed)) return '语速覆盖必须是数字';
    if (parsed < 0.5 || parsed > 2) return '语速覆盖必须介于 0.50 到 2.00 之间';
    return null;
  }, [voiceRateInput]);
  const voiceRateErrorId = 'buddy-binding-voice-rate-error';
    ...
    <section style={SS}>
      ...
      >
        ...
        >
          ...
          <div aria-live="polite" style={{ fontSize: 12, color: 'var(--fg-muted)' }}>
      ...
      {agentLoading ? (
        <div style={{ fontSize: 12, color: 'var(--fg-muted)' }}>正在读取 Agent 列表…</div>
      ) : null}
        ...
        <>
          ...
          <div style={FIELD_GROUP}>
            ...
            >
              ...
              <label style={{ display: 'grid', gap: 6 }}>
                ...
                <input
                  aria-describedby={voiceRateError ? voiceRateErrorId : undefined}
                  aria-invalid={voiceRateError ? true : undefined}
                  ...
                  style={{
                    ...IS,
                    borderColor: voiceRateError ? 'var(--danger)' : IS.borderColor,
                  }}
                  type="number"
                />
                {voiceRateError ? (
          ...
          >
            <div style={{ minWidth: 0, flex: '1 1 260px' }}>
              ...
              {selectedBinding ? (
                <div
                  style={{ marginTop: 4, fontSize: 11, lineHeight: 1.6, color: 'var(--fg-muted)' }}
                >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              ...
              <button
                type="button"
                style={BP}
                disabled={submitting || !selectedAgentId || !isDirty || Boolean(voiceRateError)}
                onClick={() => {
                  if (!selectedAgentId) {
                    return;
                  }
                  if (voiceRateError) {
                    setSubmitError(voiceRateError);
                    return;
                  }
                  setSubmitting(true);
                  setSubmitError(null);
                  ...
                  if (
                    ...
                    // 兜底：与 voiceRateError 对齐，几乎走不到（按钮已被
                    // disabled），保留以防 voiceRateError 与 input 之间出现
                    // 时序差。
                ...
                aria-disabled={
                  submitting || !selectedAgentId || !isDirty || Boolean(voiceRateError)
                }
              >
                {primaryActionLabel}
              </button>
          ...
          <div
            ...
            style={{
              fontSize: 12,
              color: submitError ? 'var(--danger)' : isDirty ? 'var(--accent)' : 'var(--fg-muted)',
            }}
          >
            {actionStatusText}
          </div>
