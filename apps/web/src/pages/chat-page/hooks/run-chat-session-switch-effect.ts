...
export interface ChatSessionSwitchDeps {
  ...
  readonly syncRecoveredStreamSnapshot: (
    session: Session,
    nextSessionStateStatus: 'idle' | 'running' | 'paused' | null | undefined,
...
export function runChatSessionSwitchEffect(deps: ChatSessionSwitchDeps): (() => void) | void {
  const {
    activateSessionView,
    activeSessionRef,
