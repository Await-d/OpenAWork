 ...
 * 通过 `<TeamSessionView/>` 复用 chat 端的消息渲染 + UnifiedComposer。
 *
 * 渲染优先级：
 *   1. `messagesOverride` 不为 undefined → 直接铺；外层负责 layout
 *      （TeamPageV2 的"对话 tab"用此路径切到 `<TeamSessionView/>` ）
 *   2. `receptionSessionId` 存在 → 内置渲染 `<TeamSessionView/>`，
 ...
 *   - 删除内置 textarea：复用 UnifiedComposer（在 TeamSessionView 内部）
...
import { TeamSessionView } from './TeamSessionView.js';
...
export interface ConversationAreaProps {
   ...
   *   - 此值非空 → 内嵌 `<TeamSessionView/>` 渲染主对话流
  ...
  /**
   * 是否启用内嵌 TeamSessionView 的 composer 输入（L1.3 inbound 反向通道 feature flag）。
   ...
   * `<TeamSessionView/>` 提供 UnifiedComposer，本字段保留只为向下兼容老调用方。
   * 新调用方无需传入此 prop。
   */
  hideInput?: boolean;
}
