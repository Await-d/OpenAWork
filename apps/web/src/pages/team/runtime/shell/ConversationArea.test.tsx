 ...
 *
 *   1. messagesOverride 注入 → 直接 pass-through，**不**渲染 TeamSessionView，
 *      也**不**渲染 idle/loading 面板。
 *   2. messagesOverride 缺省 + receptionSessionId 存在 → 内嵌 TeamSessionView
 *      （以测试 mock 形式断言其 sessionId 入参与 composerEnabled flag 透传）。
 *   3. messagesOverride 缺省 + receptionSessionId 为空 → 走 idle/loading/error
 *      引导面板，不渲染 TeamSessionView。
 *
 * TeamSessionView 自身的 Chat 渲染走 useSessionConversationState，已有专门测试，
...
// 测试期把 TeamSessionView 替换成最小桩，便于断言 sessionId / composerEnabled 透传
vi.mock('./TeamSessionView.js', () => ({
  TeamSessionView: ({
    sessionId,
    composerEnabled,
  }: {
...
describe('ConversationArea — 三态路由', () => {
  it('messagesOverride 注入时直接 pass-through，不渲染 TeamSessionView', () => {
    render(
      <ConversationArea
  ...
  it('receptionSessionId 存在 + messagesOverride 缺省 → 内嵌 TeamSessionView', () => {
  ...
  it('receptionComposerEnabled=true 时透传到 TeamSessionView', () => {
