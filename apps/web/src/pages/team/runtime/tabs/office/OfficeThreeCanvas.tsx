...
function createMonitorTexture(data: {
  ...
  return makeCanvasTexture(w, h, (ctx) => {
    ...
    // Top bar
    ctx.fillStyle = 'var(--aux, var(--aux, #8b9cf5))';
    ctx.fillRect(0, 0, w, 3);

    // Title line
    const sl = topSummary.status === '已暂停' ? 'PAUSED' : 'ACTIVE';
    const slColor = topSummary.status === '已暂停' ? 'var(--warning, #f0b429)' : 'var(--success, #3dd49a)';
    ...
    for (let i = 0; i < metricCards.length && i < 3; i++) {
      ...
      ctx.fillStyle = 'var(--fg-muted, #7b8a9e)';
      ctx.fillText(`${mc.label}`, 20, metricY + i * 16);
      ctx.fillStyle = 'var(--fg-default, #c8d1e0)';
    ...
    for (let i = 0; i < footerStats.length && i < 4; i++) {
      ...
      const barColors = ['var(--aux, var(--aux, #8b9cf5))', 'var(--success, #3dd49a)', 'var(--warning, #f0b429)', 'var(--danger, #f06b7e)'];
    ...
    const rx = 210;
    ctx.fillStyle = 'var(--success, #3dd49a)';
    ...
    const statusColors: Record<string, string> = {
      working: 'var(--aux, var(--aux, #8b9cf5))',
      discussing: 'var(--warning, #f0b429)',
      resting: 'var(--success, #3dd49a)',
    };
    ...
    for (let i = 0; i < officeAgents.length && i < 6; i++) {
      ...
      const ay = 60 + i * 14;
      ctx.fillStyle = statusColors[ag.status] ?? 'var(--fg-muted, #7b8a9e)';
      ctx.fillText(
    ...
    ctx.strokeStyle = 'var(--aux, var(--aux, #8b9cf5))';
    ctx.lineWidth = 1;
    ctx.beginPath();
    for (let i = 0; i < 20; i++) {
      const sx = 20 + i * 8;
    ...
    ctx.strokeStyle = 'var(--success, #3dd49a)';
    ctx.beginPath();
    for (let i = 0; i < 20; i++) {
      const sx = 200 + i * 8;
...
function createLabelTexture(
  ...
  const h = ch + 10;
  return makeCanvasTexture(w, h, (ctx) => {
    ctx.fillStyle = 'var(--bg-base, #080b12)';
    ctx.globalAlpha = 0.6;
    ctx.fillRect(0, 0, w, h);
    ctx.globalAlpha = 1;
    ...
    ctx.textBaseline = 'top';
    ctx.fillStyle = isHovered ? 'var(--aux, var(--aux, #8b9cf5))' : 'var(--fg-on-accent, #ffffff)';
...
function addWorkstation(scene: THREE.Scene, x: number, z: number, faceAngle: number) {
  ...
  const scrTex = makeCanvasTexture(160, 100, (ctx) => {
    ...
    const lineColors = ['var(--aux, var(--aux, #8b9cf5))', 'var(--success, #3dd49a)', 'var(--fg-muted, #7b8a9e)', 'var(--warning, #f0b429)', 'var(--aux, var(--aux, #8b9cf5))', 'var(--success, #3dd49a)'];
    ...
    ctx.fillStyle = 'var(--danger, #f06b7e)';
    ctx.fillRect(4, 2, 3, 3);
    ctx.fillStyle = 'var(--warning, #f0b429)';
...
function buildOffice(
  ...
  addFloorLabel('☕ REST', -ROOM_W / 2 + restW / 2, ROOM_D / 2 - restD / 2, 'var(--success, #3dd49a)');
  ...
  addFloorLabel('💬 DISCUSS', tableX, tableZ - tableD / 2 - 0.3, 'var(--warning, #f0b429)');
  ...
  addFloorLabel('💻 WORK', 0, aisleZ + 0.5, 'var(--aux, var(--aux, #8b9cf5))');
      ...
      if (projScreen && projScreen.material instanceof THREE.MeshStandardMaterial) {
        ...
        if (slideTimerRef.current >= 240) {
          ...
          const slideTex = makeCanvasTexture(320, 200, (ctx) => {
            ...
            ctx.fillStyle = 'var(--fg-on-accent, #ffffff)';
            ...
            // Chart bars
            const barColors = ['var(--aux, var(--aux, #8b9cf5))', 'var(--success, #3dd49a)', 'var(--warning, #f0b429)'];
            for (let b = 0; b < 3; b++) {
