...
import SkillRecommendationDrawer from '../recommendation/SkillRecommendationDrawer.js';
import {
...
function Group({
  ...
  return (
    <section style={{ ...PANEL, marginBottom: 12 }}>
      ...
      {rows.length === 0 ? (
        <div style={{ color: 'var(--fg-muted)', fontSize: 13, padding: 8 }}>—</div>
      ) : (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {rows.map((row) => (
            <div key={row.skillId} style={ROW}>
              ...
              <span style={{ fontSize: 11, color: 'var(--fg-muted)' }}>始终可用</span>
            </div>
          ))}
        </div>
      )}
