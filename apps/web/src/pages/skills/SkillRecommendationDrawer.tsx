...
function deltaColor(origin: RowDecision['origin']): string {
  if (origin === 'recommended-only') return 'var(--success, #3dd49a)';
  if (origin === 'current-only') return 'var(--danger, var(--danger, #f06b7e))';
  return 'var(--text-3)';
}
    ...
    <div style={DRAWER_BACKDROP} onClick={onClose} role="dialog" aria-modal="true">
      <div style={DRAWER_PANEL} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={BODY}>
            ...
            {recommendation ? (
              <span
                style={{
                  ...
                  color: recommendation.fellBackToHeuristic ? 'var(--warning, #f0b429)' : 'var(--accent)',
                  alignSelf: 'center',
                }}
              >
          ...
          {error ? (
            <div style={{ color: 'var(--danger, var(--danger, #f06b7e))', fontSize: 12, marginBottom: 8 }}>{error}</div>
          ) : null}
          ...
          {totalRows > 0 ? (
            <>
              <div style={STAT_CARD} aria-label="推荐结果汇总">
                ...
                <div>
                  <div style={{ ...STAT_VALUE, color: 'var(--success, #3dd49a)' }}>+{summary.added}</div>
                  <div style={STAT_LABEL}>新增</div>
                </div>
                <div>
                  <div style={{ ...STAT_VALUE, color: 'var(--danger, var(--danger, #f06b7e))' }}>-{summary.removed}</div>
