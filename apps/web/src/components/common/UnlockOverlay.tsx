  ...
  return (
    <div
      ...
      style={{
        ...
        inset: 0,
        zIndex: 9999,
        background: 'color-mix(in srgb, var(--bg, var(--bg-base, #080b12)) 96%, black)',
        display: 'flex',
        alignItems: 'center',
    ...
    >
      ...
      >
        ...
        <div style={{ minHeight: 18, fontSize: 12, color: 'var(--danger, var(--danger, var(--danger, #f06b7e)))' }}>
          {lockoutSeconds > 0 ? (
            <span>已锁定，剩余 {formatLockoutHint(lockoutSeconds)}</span>
          ) : error ? (
            error
          ) : verifying ? (
            <span style={{ color: 'var(--text-3)' }}>验证中…</span>
          ) : null}
        </div>
