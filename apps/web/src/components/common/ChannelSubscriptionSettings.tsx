...
function getStatusTone(status: ChannelEditorStatus): { label: string; color: string } {
  switch (status) {
    case 'connected':
      return { label: '运行中', color: 'var(--success, var(--success, #3dd49a))' };
    case 'error':
      return { label: '异常', color: 'var(--danger, var(--danger, #f06b7e))' };
    case 'pending':
      return { label: '待创建', color: 'var(--warning, var(--warning, #f0b429))' };
    default:
      return { label: '已停止', color: 'var(--fg-muted, #7b8a9e)' };
  }
}
                  ...
                  {(activeDescriptor?.configSchema ?? []).map((field) => (
                    <div key={field.key} className="channel-field">
                      <div className="channel-field__label">
                        {field.label}
                        {field.required ? <span style={{ color: 'var(--danger, var(--danger, #f06b7e))' }}>*</span> : null}
                      </div>
                      {field.description ? (
