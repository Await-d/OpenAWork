  ...
  return (
    ...
    >
      <div style={TABS_BAR}>
        ...
        {loading ? (
          <span style={{ fontSize: 11, color: 'var(--text-3)', alignSelf: 'center' }}>加载中…</span>
        ) : null}
      </div>
...
function Group({
  ...
  return (
    <section style={SECTION}>
      ...
      {rows.length === 0 ? (
        <div style={{ color: 'var(--text-3)', fontSize: 11, padding: 4 }}>—</div>
      ) : (
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            border: 'none',
            background: 'transparent',
            color: canReset ? 'var(--accent)' : 'var(--text-3)',
          ...
          {visible.map((row) => {
            ...
            return (
              ...
              >
                ...
                {isBuiltin ? (
                  <span style={{ fontSize: 10, color: 'var(--text-3)' }}>始终可用</span>
                ) : (
                  <>
                    <label style={{ fontSize: 10, display: 'flex', alignItems: 'center', gap: 3 }}>
                      <input
  ...
  return (
    ...
    >
      <div>
        ...
        <span style={{ color: 'var(--text-3)' }}>：当前作用域是否可被模型发现 / 调用</span>
      </div>
      <div>
