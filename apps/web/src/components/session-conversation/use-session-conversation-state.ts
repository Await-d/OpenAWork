...
import type { ChatMessage, ChatMessagePart } from './runtime/support.js';
import { normalizeChatMessages } from './runtime/support.js';
import type { SessionStateStatus, SessionTodoItem } from './runtime/session-runtime.js';
import type { ChatBackendUsageSnapshot } from './runtime/stream-usage.js';
import type { StreamingThinkingBlock } from './runtime/streaming-thinking.js';
...
export interface SessionConversationState {
  ...
  /**
   * sessions.substate（L1.3 改造 2 待做）：当前子状态机位置。
...
export function useSessionConversationState(
  ...
  const { sessionId, gatewayUrl, token, enabled = true, defaults } = options;
  ...
  const [stoppingStream, _setStoppingStream] = useState(false);
  const [streamBuffer, _setStreamBuffer] = useState('');
  const [streamThinkingBuffer, _setStreamThinkingBuffer] = useState('');
  const [streamThinkingBlocks, _setStreamThinkingBlocks] = useState<StreamingThinkingBlock[]>([]);
  const [streamingSegments, _setStreamingSegments] = useState<ChatMessagePart[]>([]);
  const [reportedStreamUsage, _setReportedStreamUsage] = useState<ChatBackendUsageSnapshot | null>(
    null,
  );
  ...
  const [hasPendingFollowContent, _setHasPendingFollowContent] = useState(false);
