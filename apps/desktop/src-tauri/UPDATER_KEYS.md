...
| Secret | Description |
|---|---|
| `TAURI_SIGNING_PRIVATE_KEY` | Full private key text generated by `tauri signer generate`, including `untrusted comment:` and `trusted comment:` lines. Base64-wrapping that full text is also accepted by the release workflow. |
| `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` | Password entered when generating the Tauri updater key (leave empty if none) |
