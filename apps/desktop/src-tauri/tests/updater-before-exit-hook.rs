...
fn proxy_update_path_rebinds_before_exit_hook_to_stop_gateway_child() {
    ...
    assert!(proxy_update_section
        .contains("shutdown_gateway_child_from_state(&gateway_process_for_updater);"));
...
fn updater_plugin_builder_stays_on_default_build_path() {
    ...
    assert!(
        run_section.contains("let updater_plugin = tauri_plugin_updater::Builder::new().build();")
    );
